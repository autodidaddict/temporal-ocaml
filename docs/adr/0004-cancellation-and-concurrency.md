...
Cancelling a running workflow asks it to stop cleanly. The workflow receives the
request and chooses how to respond. It can run cleanup such as refunds or
compensations, or it can decline and complete normally. Termination is
the forced alternative. It stops the execution immediately, and the workflow code
never observes it. Every official SDK models cancellation cooperatively. Python
catches `asyncio.CancelledError` and can "perform some cleanup … or … deny the
cancellation entirely". Go closes `ctx.Done()` and blocking calls return
`*CanceledError`. TypeScript and Java raise `CancelledFailure`/`CanceledFailure` at
await points. Signals push input into a running workflow and queries read its state
(ADR-0002). Cancellation is a third such interaction, asking the execution to wind down.

Cancellation requires concurrency. The runtime today runs a single logical thread.
`execute_activity`, `execute_child_workflow`, and `sleep` each block the body until
they resolve. A blocked body cannot observe a request to abandon the operation it is
waiting on, and it cannot express "wait for a signal or a timeout, whichever comes
first." Cancellation has meaning only when operations are in flight concurrently and
a second thread of control can interrupt them. This ADR therefore covers both in
dependency order. It defines a deterministic concurrency model first, built from
futures and a cooperative scheduler, then adds cancellation on top.

This revisits a simplification ADR-0002 made deliberately. That ADR rejected the
channel model partly to keep the body "deterministic, not concurrent," and closed by
noting a limitation it left open: "within a single activation the exact ordering of a
handler firing between two synchronous statements is coarser than a true coroutine
scheduler … Documented, not solved." A cooperative scheduler keeps the body
deterministic while making it concurrent, and resolves that limitation.

### What the other SDKs do

**Concurrency.** Every SDK exposes cooperative, single-threaded concurrency driven
by a deterministic scheduler:

- **Go:** `workflow.Go(ctx, f)` spawns a coroutine; `workflow.ExecuteActivity(...)`
  returns a `Future`; `future.Get(ctx, &out)` awaits; `workflow.NewSelector(ctx)`
  with `.AddFuture/.AddReceive/.AddDefault/.Select(ctx)` races heterogeneous
  awaitables.
- **TypeScript:** activities and child workflows return `Promise`s; `Promise.all` and
  `Promise.race`; `sleep(ms)`; `condition(fn)` and `condition(fn, timeout)` (the
  latter returns a `boolean`: `true` if met before the timeout).
- **Java:** `Async.function/procedure` returns a `Promise`; `Promise.allOf` and
  `Promise.anyOf`; `Workflow.await(supplier)` and `Workflow.await(timeout, supplier)`
  (returns `boolean`: `false` if timed out).
- **Python:** durable asyncio, with `asyncio.gather` / `wait` / `as_completed` over
  activity awaitables, plus `workflow.wait_condition(fn, timeout=…)` (returns `None`,
  raises `asyncio.TimeoutError` on timeout).

The scheduler must be deterministic. Go's dispatcher "executes coroutines one by one
in deterministic order until all of them are completed or blocked"
(`Dispatcher.ExecuteUntilAllBlocked`), and command sequence numbers come from a
single monotonic counter advanced in that fixed order. Deterministic run order plus
one shared counter give the same logical command the same seq on every replay, even
across concurrent coroutines. Every SDK forbids native goroutines, threads, and
`setTimeout` inside a workflow for this reason.

**Cancellation** uses a tree of **cancellation scopes** whose root is the workflow
itself:

- **Scopes.** TS `CancellationScope` (`cancellable` / `nonCancellable` /
  `withTimeout` / `.run` / `.cancel()` / `.cancelRequested`); Go
  `workflow.WithCancel(ctx) → (ctx, cancel)`, where the child `Done` channel closes
  when `cancel()` is called or the parent is cancelled; Java
  `Workflow.newCancellationScope` / `newDetachedCancellationScope`, `scope.cancel()`,
  `isCancelRequested()`. Cancelling a scope propagates to every activity, timer, and
  child workflow started within it.
- **Cleanup after cancel** runs in a detached scope that ancestor cancellation does
  not reach: Go `workflow.NewDisconnectedContext`, TS
  `CancellationScope.nonCancellable`, Java `newDetachedCancellationScope`, Python
  `asyncio.shield`.
- **Activity cancellation type** governs how the workflow-side call resolves when an
  ...
  defaults are `TRY_CANCEL`. Go's `ActivityOptions.WaitForCancellation` defaults to
  `false`, which behaves like `TRY_CANCEL`, and Go has no `ABANDON`. TypeScript
  defaults to `WAIT_CANCELLATION_COMPLETED`.
- **Timers and child workflows** resolve their await as a cancellation error when
  cancelled (Go `Sleep` returns `*CanceledError`; TS `sleep` throws
  `CancelledFailure`). `ParentClosePolicy`, already in the SDK, governs a separate
  case: what happens to a child when the parent closes.
...
2. **The re-run model.** The SDK does not persist continuations across workflow
   ...
   history-ordered event log via a replay cursor. Concurrency and cancellation live
   inside that model.
...
4. **Preserve ADR-0002 delivery.** History-ordered signal and update delivery carries
   forward unchanged; concurrency generalizes it.

## Decision

Add, in two layers over the existing runtime:

1. **Deterministic in-workflow concurrency.** A `future` type, non-blocking `start_*`
   forms, `await` / `await_all` / `await_any`, `async` (spawn), and a `wait_condition`
   timeout, driven by a small deterministic cooperative scheduler that generalizes
   ADR-0002's single replay cursor.
...
stays unchanged; each becomes `await` of the corresponding `start_*`.

### Part 1: deterministic concurrency
...
val async : _ ctx -> (unit -> 'a) -> 'a future       (* spawn a fiber; workflow.Go / Async.function *)
...
Heterogeneous racing, a Go `Selector` over differently-typed futures, is expressible
as `wait_condition` over each future's resolved state; a first-class `select` is left
to a follow-up (see Open questions).

#### Runtime: from a single cursor to a cooperative dispatcher

ADR-0002 walks a single cursor over the history log, driven by the one body's
demands. Concurrency replaces the single walker with a scheduler that drives the log
and many fibers:

- **Fibers.** The main body is the root fiber. `async` registers a new fiber (an
  effectful thunk) with the scheduler. All fibers run under the same `Effect.Deep`
  handler.
- **A deterministic ready queue.** The scheduler holds runnable fibers FIFO in
  ...
  until it blocks or completes. This is Go's `ExecuteUntilAllBlocked`. A fiber blocks
...
- **Emit-once.** A `start_*` emits its command only if the operation has not already
  been issued and its resolution is not yet in the log. The scheduler records issued
  seqs in `run_state`, so a re-run triggered by an unrelated job, such as a signal
  arriving while three activities are outstanding, does not re-issue them. This
  tightens the current per-effect "resolution present?" check, which re-emits an
  outstanding operation's start on every re-run. That is tolerable when only one
  operation is outstanding but incorrect when several are.

With a single fiber and no `async`, this reduces to ADR-0002's cursor walk. One
...
1. **Fibers run in a fixed order.** The ready queue is FIFO in deterministic creation
   order, and each fiber runs to its next block point. `async` spawns are themselves
...
2. **Sequence numbers follow that order.** The runtime advances the existing per-type
   ...
   issued, in the deterministic scheduler order. This is the guarantee Go gets from
   its single monotonic command-id counter. The same logical operation gets the same
   seq every time, and history-matching holds.

Rather than reuse Eio's scheduler, the SDK builds its own over the effect handler.
Eio's scheduler is wall-clock and non-deterministic and would break replay, the same
reason ADR-0002 declined to back signals with `Eio.Stream`. The resulting
concurrency is deterministic and driven by history.
...
val with_cancel_scope : 'i ctx -> ('i ctx -> cancel:(unit -> unit) -> 'a) -> 'a
   (* run [f] in a fresh cancellable child scope. [cancel ()] cancels every operation
      started under [ctx'], raising [Canceled] in fibers awaiting them. Mirrors Go
      workflow.WithCancel, TS CancellationScope.cancellable, Java newCancellationScope. *)

val with_timeout : 'i ctx -> float -> ('i ctx -> 'a) -> 'a
   (* run [f] in a scope that auto-cancels after [seconds] (TS CancellationScope.withTimeout). *)

val detached : 'i ctx -> ('i ctx -> 'a) -> 'a
   (* run [f] detached: ancestor cancellation does not propagate in, so post-cancel
      cleanup completes. Mirrors Go NewDisconnectedContext, TS nonCancellable, Java
      newDetachedCancellationScope, Python asyncio.shield. *)
...
An activity with a deadline, plus its cleanup, then reads:
...
- **Recorded in history, so replay-safe.** The cancellation trigger is always a
...
- **`Wait_cancellation_completed`**: emit `RequestCancelActivity`, then resolve with
  ...
  functions. Until heartbeating exists, the request only takes effect for an
  activity that has not yet started.
...
- **Poll-only cancellation, no concurrency.** Deliver `CancelWorkflow` as a flag the
  ...
  in-flight operations. Rejected: it cannot cancel a running activity, timer, or
...
- **Persisted continuations (sticky resume).** Rather than re-run the body from the
  ...
  activations, as the real SDKs' runtimes do. Rejected: it abandons the project's defining
...
- **Eio fibers for the scheduler.** Rejected: Eio's scheduler is non-deterministic
...
- **Forced (non-cooperative) cancellation.** Rejected: Temporal cancellation is
  cooperative by definition. A forced kill is termination, a server-side action with
  no workflow-side API.
- **`select`/`Selector` as the primary racing API** (Go-style). Deferred rather than
  rejected. `await_any` and `wait_condition` cover homogeneous and state-based racing,
  and a typed heterogeneous `select` can be added later over the same futures without
  reshaping the model.
